...
class CountDownCustomCubitPage extends StatefulWidget {
  const CountDownCustomCubitPage({super.key, required this.seconds});
...
class TimerCubit extends CustomCubit<int> {
  ...
  void _onStart() {
    ...
    _timeSubscription =
        Stream.periodic(const Duration(seconds: 1), (computationCount) => startTime - computationCount).listen(
      (time) {
        _setTime(time);
        if (time == 0) {
          _onFinish();
