...
class MyWidgetTongHop extends StatelessWidget {
  const MyWidgetTongHop({super.key});
...
class ListTileSelectExampleState extends State<ListTileSelectExample> {
  ...
  late List<bool>
      _selected; // danh sách các bool được sử dụng để theo dõi xem mỗi mục có được chọn hay không.
  bool _selectAll = false;
  bool _isGridMode = false; // xác định show Grid hay List

  @override
  void initState() {
    super.initState();
    initializeSelection(); // khởi tạo danh sách _selected.
  }
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          leading: isSelectionMode
              ? IconButton(
                  tooltip: 'Disable Selection Mode',
              ...
              : const SizedBox(),
          actions: <Widget>[
            // thực hiện các action trên các button
            if (_isGridMode)
              IconButton(
                tooltip: 'Switch Grid/List',
        ...
        // _isGridMode xác định xem ứng dụng đang ở chế độ lưới (GridView) hay không - quyết định GridBuilder hay ListBuilder
        body: _isGridMode
            ? GridBuilder(
