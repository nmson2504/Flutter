...
class CircularProgressIndicatorApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const CircularProgressIndicator(
      ...
      strokeWidth: 8, // độ dày đường vẽ cung tròn
    );
  }
}
...
class CircularProgress02AppState2 extends State<CircularProgress02> {
  ...
  void _updateProgress() {
    const oneSec =
        Duration(milliseconds: 400); // set speed chuyen dong hieu ung
    Timer.periodic(oneSec, (Timer t) {
      // import 'dart:async'
      setState(() {
        _progressValue +=
            0.2; // set số part chuyen đong - 0.1: 10 part, 0.2: 5 part
                    ...
                    setState(() {
                      ...
                      if (determinate) {
                        ...
                        controller
                          ...
                          ..repeat();
/* 
Đoạn mã controller..forward(from: controller.value)..repeat(); sử dụng Dart's cascade operator (..) để gọi hai phương thức trên cùng một đối tượng mà không cần phải viết lại tên của đối tượng nhiều lần.
controller..forward(from: controller.value): Đoạn này gọi phương thức forward trên đối tượng controller. Phương thức này thường được sử dụng để khởi động một animation. Bằng cách truyền giá trị from là controller.value, animation sẽ bắt đầu từ giá trị hiện tại của controller, giúp tránh hiệu ứng nhảy giữa các trạng thái.
..repeat(): Sau khi đã gọi phương thức forward, dấu .. tiếp tục thực hiện gọi phương thức repeat trên cùng đối tượng controller. Phương thức này đặt animation vào chế độ lặp lại, nghĩa là nó sẽ tự động lặp lại khi hoàn thành. */
                      }
                    });
                  },
                ),
